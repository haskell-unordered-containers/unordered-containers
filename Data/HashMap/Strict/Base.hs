...
insertWith :: (Eq k, Hashable k) => (v -> v -> v) -> k -> v -> HashMap k v
           -> HashMap k v
insertWith f k0 v0 m0 = go h0 s0 k0 v0 0 m0
  where
    ...
    go h s k x bs (Full ary) =
        ...
        in Full ary'
      where i = index h bs
    go h s k x bs t@(Collision hx sx hm)
        | h == hx   = Collision hx sx $ go (hashWithSalt sx k) sx k x 0 hm
  ...
  where
    ...
    go h s k bs (Full ary) =
        let i    = index h bs
            ...
            ary' = update16 ary i $! st'
        in Full ary'
    go h s k _ t@(Collision hx sx hm)
        | h == hx   = Collision hx sx $ go (hashWithSalt sx k) sx k 0 hm
        | otherwise = t
{-# INLINABLE adjust #-}
